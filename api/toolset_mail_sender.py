...
class ToolsetMailSender(BaseMailSender):

    def __init__(self) -> None:
        super().__init__()
        self.host = os.getenv("web_host")  # use env variables - web_host

    def _count_test_apis(self, project: str, env: str) -> int:
    ...
    def _set_content(self, data: dict, project: str, env: str) -> None:
        ...
        count_apis = self._count_test_apis(project=project, env=env)
        ...
        content += f'<p>Toolset Test Result for <font color="#1E90FF"><b>{site} {project}</b></font>: <font color="{result_string_color}"><b>{subject_test_result}</b></font></p>'
        content += f'<p><font color="#1E90FF"><b>Total test APIs:</b></font> {count_apis} </p>'
        content += f'<p>'
        ...
        content += f'<p><font color="#47B0A6"><b>Testing Report:</b></font> <a target="_blank" href="http://{self.host}/{project}/{env}/index.html">Allure Test Report</a></p>'
